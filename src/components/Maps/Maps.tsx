	...
	const SweetAlert__HidingTips = async ({
		...
		await reactSwal.fire({
			...
			html: (
				<div>
					{obj.map((item, index) => (
						<div key={`item-tips-` + index}>
							<h5 style={{ fontSize: 22 }}>{item.title}</h5>
							<ul>
								{item.lists.map((list, index) => (
									<li key={`item-tips-list-` + index}>
										{list.length === 0 ? "N/A" : list}
									</li>
								))}
							</ul>
						</div>
			...
			html: (
				<div>
					...
					<div className="d-flex justify-content-center align-items-center flex-wrap">
						{obj[0].map((item, index) => (
							<div key={`item-schema-` + index} className="schemas">
					...
					<div className="d-flex justify-content-center align-items-center flex-wrap">
						{obj[1].map((item, index) => (
							<div key={`item-legend-` + index} className="legends">
...
export default function Maps(): T_FunctionComponent {
	return (
		<div className="maps">
